...
public:
    ...
    void TearDown() override { /* implementation */ }
};
...
{
    fetchers::Settings fetchSettings = fetchers::Settings{};
    fetchSettings.accessTokens.get().insert({"sourcehut.com","token"});
    fetchSettings.accessTokens.get().insert({"sourcehut.com/a/b","another_token"});
    auto i = Input::fromURL(fetchSettings, "sourcehut:a/b");

    auto token = i.scheme->getAccessToken(fetchSettings, "sourcehut.com", "sourcehut.com/a/b");
    EXPECT_EQ(token,"another_token");

    token = i.scheme->getAccessToken(fetchSettings, "sourcehut.com", "sourcehut.com/a/c");
    EXPECT_EQ(token,"token");
}
